...
// app.use(cors({
//     origin: [ "https://www.stawro.com", "https://stawro.com", "http://localhost:3000"],
...
//         const user12 = await Usermodule.findOne({email})

//         const find_otp = await User_s_OTP_module.findOne({username})
//                 if(find_otp || user12.valid === "no"){
//                     if(find_otp){
//                         await find_otp.deleteOne();
//                     }
                    
//                     if(user12.valid === "no"){
//                         await user12.deleteOne();
//                     }
//                 }
...
//                         const data = await User_s_OTP_module.create({Time, username, otp : OTP})
...
//                                               Please verify your email using the OTP. After verification, 
//                                               you can access the website's login page by clicking on Login.</p><br/>
//                                               <span>OTP : <strong>${data.otp}</strong></span><br/>
//                                                 <a href = "https://www.stawro.com/login" style="text-decoration: none;">Login</a>
//                                             <strong></strong><br/>
...
//                     if (error) {
//                         console.error("Error sending email:", error);
//                         return res.status(400).json({ Status: "EMAIL_ERR", message: "Failed to send email." });
//                     }
...
app.post("/logout/data/app", async (req, res) =>{
    ...
    try{
        ...
        if(get_data){
            ...
            const user = await Usermodule.findOne({_id : data})
            if(user){
...
app.post('/login/data/app', async (req, res) => {
    ...
    try {
        ...
        await FCMModule.findOneAndUpdate(
            { user: user.username },
            { Time: new Date(), user: user.username, email: user.email, FCM, user_id: user._id },
...
app.get("/get/coins/by/id/cupons/by/apps/:id", async (req, res) => {
    ...
    try {
        // First, check Claimrequestmodule
        let from = "Request";
        let data = await Claimrequestmodule.findOne({ ID: id });
        

        // If not found, check ClaimedCoinsmodule
        if (!data) {
            data = await ClaimedCoinsmodule.findById(id);
            from = "Claimed";
        }
...
app.get("/kerthi/data/:valid", async (req, res) =>{

   try{
        const data = await Usermodule.findOne({valid : req.params.valid})
        if(data){
            return res.status(200).json({data})
        }
        else{
  ...
  app.post("/get/new/notification/fcm/token/from/app", async (req, res) => {
    ...
    try {
      ...
      if (notificationData.tokens.includes(fcm)) {
        console.log(`⚠️ This ${fcm} FCM Token already exists`);
        return res
          .status(201)
...
const ResetOTPSchema = new mongoose.Schema(
  ...
  // Schema for User (Assuming you have a User model)
